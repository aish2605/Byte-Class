...
import JavacDatabase from "./JavacDatabse";

const Javac = () => {
  const [serviceData] = useState(JavacDatabase);
  ...
  return (
    <>
      ...
      <section className="course-section">
        <h1 className="course-heading">Java Roadmap</h1>
        <div className="card-grid">
          {serviceData.map(({ id, logo, title, info, link }) => (
            <div className="course-card" key={id}>
              ...
              <a href={link} className="course-button" target="_blank" rel="noreferrer">Explore</a>
            </div>
          ))}
        </div>
      </section>
